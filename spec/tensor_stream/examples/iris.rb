...
run_test = ->(nn, inputs, expected_outputs) {
  ...
  inputs.each.with_index do |input, i|
    ...
    prediction_success.call(output, expected_outputs[i]) ? success += 1 : failure += 1
...
puts "Untrained classification success: #{success}, failure: #{failure} (classification error: #{error_rate.call(failure, x_test.length)}%, mse: #{(avg_mse * 100).round(2)}%)"
...
puts "Trained classification success: #{success}, failure: #{failure} (classification error: #{error_rate.call(failure, x_test.length)}%, mse: #{(avg_mse * 100).round(2)}%)"
