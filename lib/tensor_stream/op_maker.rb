module TensorStream
  ##
  # Class that manages the TensorStream Op DSL
  class OpMaker
    ...
    def other_names(*aliases)
      @aliases += aliases.flatten.map(&:to_s)
    end
    ...
    def self.each_op(&block)
      @ops.values.sort_by(&:operation).reject(&:exclude).each do |op|
        block.call(op)
      end
    end
    ...
    def generate_body
      ...
      body << if custom_post.empty?
                "_op(:#{operation}, #{(expand_params(false) + options_call).join(', ')})"
              else
                "result = _op(:#{operation}, #{(expand_params(false) + options_call).join(', ')})"
              end
    ...
    def supports_broadcasting!
      raise "Ops with parameters < 2 cannot support broadcasting" if @parameters.size < 2
